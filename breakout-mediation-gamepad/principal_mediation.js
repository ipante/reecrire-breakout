...
scene("jeu", () => {
   ...
   // test de la présence d'un gamepad
   if(getGamepads().length === 0){
      // s'il n'y a pas de gamepad, on utilise la souris
      onUpdate("paddle", (p) => {
         p.pos.x = mousePos().x;
      });
   }
   else{
      // on utilise le gamepad
   ...
   // si le gamepad se déconnecte, on repasse à la souris
   onGamepadDisconnect(() => {
      onUpdate("paddle", (p) => {
         p.pos.x = mousePos().x;
      });
...
scene("gameover", ({ score }) => {
   ...
   // ajout de la gestion du gamepad
   onGamepadButtonPress(() => {
      go("accueil");
   });
});
